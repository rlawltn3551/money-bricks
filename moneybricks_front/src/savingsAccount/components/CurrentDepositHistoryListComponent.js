...
import { getCurrentDepositHistory } from "../api/accountApi";
...
import "../styles/DepositHistoryLIstComponent.scss";
...
const CurrentDepositHistoryListComponent = () => {
	...
	useEffect(() => {
		const fetchDepositHistoryList = async () => {
			setIsLoading(true);
			try {
				const data = await getCurrentDepositHistory({ page, size });
	...
	return (
		<div>
			<h2 className="deposit-history-title">현재 입금 내역</h2>
			<div className="view-toggle">
				...
				<button onClick={() => navigate("/mypage/deposit-history/all/list")}>
					모든 입금 내역 보기
				</button>
			</div>
			...
			{depositHistoryList != null && depositHistoryList.length > 0 ? (
				viewMode === "table" ? (
					<>
						...
						<p className="total-deposits">총 입금 횟수: {depositHistoryList[0].depositCount} 회</p>
						{/* 페이지네이션 */}
...
export default CurrentDepositHistoryListComponent;