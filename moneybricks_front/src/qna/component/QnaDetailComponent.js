...
const QnaDetailComponent = ({ qno }) => {
	...
	const handleReplySubmit = () => {
		if (!replyText || !replyText.trim()) return;  // ✅ replyText가 undefined인지 체크

		console.log("로그인 상태 확인:", loginState);

		const userRoles = loginState?.memberRoles || [];  // ✅ undefined 방지

		console.log("사용자 역할:", userRoles);

		if (!Array.isArray(userRoles)) {
			console.error("🚨 memberRoles 값이 배열이 아닙니다:", userRoles);
			alert("잘못된 로그인 정보입니다.");
			return;
		}

		if (!userRoles?.includes("ADMIN")) {  // ✅ includes() 실행 전에 undefined 체크
